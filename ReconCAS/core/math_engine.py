...
class MathEngine:
    ...
    def format_input(text: str) -> str:
        ...
        # Soru sonundaki '= ?', '=?', '=_', '=-', '= ...' veya tek '=\s*$' temizliği
        # (Denklemin her iki tarafı doluysa ör. 'x + 5 = 10' dokunma; sadece sağ tarafı boş veya soru olanları temizle)
        text = re.sub(r'=\s*(?:\?|_|\.+|-+)?\s*$', '', text)
        # Kalan tekil soru işaretlerini de temizle (ör. '27 + 3 ?' -> '27 + 3')
    ...
    def is_advanced(text: str) -> bool:
        try:
            formatted = MathEngine.format_input(text)
            # Eşitsizlikler her zaman gelişmiştir
            if any(op in formatted for op in ['>', '<', '>=', '<=']):
                return True
            # Eşitlik varsa ve her iki tarafında serbest değişken içeriyorsa
            if '=' in formatted:
                parts = formatted.split('=', 1)
                ...
                if left_str and right_str:
                    left = parse_expr(left_str, transformations=MathEngine.TRANSFORMATIONS)
                    right = parse_expr(right_str, transformations=MathEngine.TRANSFORMATIONS)
                    if len(left.free_symbols) > 0 or len(right.free_symbols) > 0:
                        return True
                    return False
            # Eşitlik yoksa serbest sembol kontrolü
            MathEngine._lexical_validation(formatted)
            parsed = parse_expr(formatted, transformations=MathEngine.TRANSFORMATIONS)
