...
class TerminalGUI(tk.Tk):
    def __init__(self, user_id):
        ...
        self.is_root = DatabaseEngine.is_root(self.user_id)
        if self.is_root:
            self.title("SYSTEM.CORE // V0.1 ENTERPRISE [ROOT]")
    ...
    def setup_main_menu(self):
        ...
        if self.is_root:
            tk.Label(header, text=">_ AUTH: ROOT_OPERATOR // GOD_MODE_ACTIVE", font=("Consolas", 9, "bold"), fg="#FFD700", bg=THEME["bg"]).pack()
        else:
        ...
        if self.is_root:
            tk.Button(self, text="[ 👑 ROOT_ADMIN_PUPPET_CONSOLE ]",
                      command=self.open_admin_console,
                      font=("Consolas", 9, "bold"), bg="#311B92", fg="#FFD700",
                      relief="solid", bd=1, activebackground="#4A148C").pack(pady=4, padx=20, fill="x")
    ...
    def open_admin_console(self):
        AdminPuppetWindow(self, self.user_id)
